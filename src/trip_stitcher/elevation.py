...
class ElevationOracle:
    def __init__(self):
        ...
        self._request_session = requests_cache.CachedSession(
            cache_name="api_cache", expire_after=3600 * 24 * 30
        )
    ...
    def get_elevation(self, latitude: list[float], longitude: list[float]) -> list[float]:
        ...
        while len(latitude) > 0 and len(longitude) > 0:
            ...
            location_string = "?locations=" + "|".join(
                f"{lat},{lon}" for lat, lon in zip(lat_chunk, lon_chunk)
            )
